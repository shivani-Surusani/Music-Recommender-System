...
cursor.execute('CREATE TABLE songs (userid VARCHAR(30), timestamp VARCHAR(300), artistid VARCHAR(50), artistname VARCHAR(300), trackid VARCHAR(500), trackname VARCHAR(300))')
cursor.execute('CREATE TABLE songindex (ind INT AUTO_INCREMENT, trackid VARCHAR(500), PRIMARY KEY(ind))')
...
cursor.execute('CREATE TABLE similarity (userid1 VARCHAR(30) , userid2 VARCHAR(30))')
...
import string
songlist={}
with open('../../data/userid-timestamp-artid-artname-traid-traname-trim.tsv','r') as songs:
	songs=csv.reader(songs,delimiter='\t')
	temp=next(songs)
	for song in songs:
		if song[6] == '' :
			#~ print song
			continue
		if song[6] not in songlist:
			songlist[song[6]]=1
			#~ print song
			q= 'INSERT INTO songindex (trackid) VALUES(\''+str(song[4])+'\')'
			#~ print q
			cursor.execute(q);
		for i in range(0,6):
